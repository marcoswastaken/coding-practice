...
class SubMessage(object):
    ...
    def apply_transpose(self, transpose_dict):
        ...
        on the dictionary
        '''
        # Initialize an empty string to store the transposed message
        transposed_message = ''

        # For each character in the message_text, append the transpose if the
        # character is in the transpose_dict, otherwise append the character
...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        '''
        # Make a list of all of the permutations of the vowels
        vowel_permutations = get_permutations('aeiou')
        best_yet = (0, 'aeiou')

        # For each of those permutations
        for permutation in vowel_permutations:
            # Reset the count of valid words found,
            # and the potential word
            valids_found = 0
            potential_word = ''

            # Get the result of applying this permutation
            permuted_message = self.apply_transpose(
                self.build_transpose_dict(permutation))

            # Count the valid words that appear in the permuted message
            # by checking whether the substrings that appear between
            # non-alphabet characters are valid words

            for char in permuted_message:
            ...
            if best_yet[0]< valids_found:
                best_yet = (valids_found, permutation)

        return self.apply_transpose(self.build_transpose_dict(best_yet[1]))
